package gorsa

import (
	"errors"
	"fmt"
	"io/ioutil"
	"log"
	"regexp"
	"testing"
)

func IsInt(str string) bool {
	match, _ := regexp.MatchString(`^(0|[1-9][0-9]*)$`, str)
	return match
}
func ReadSslFileToStr(path string) (string, error) {
	buf, err := ioutil.ReadFile(path)
	if err != nil {
		return "", err
	}
	return string(buf), nil
}

func TestRSA(t *testing.T) {
	t.Log("start")
	// publicKey, publicKeyErr := ReadSslFileToStr("/Users/osmeteor/go/src/partnerPortalBackendApi/conf/PartnerServerSsl/KeyroadsPatner_server.crt")
	// if publicKeyErr != nil {
	// 	t.Log("---->", publicKeyErr)
	// }
	// privateKey, privateKeyErr := ReadSslFileToStr("/Users/osmeteor/go/src/partnerPortalBackendApi/conf/PartnerServerSsl/KeyroadsPatner_server.key")
	// if privateKeyErr != nil {
	// 	t.Log("----1>", privateKeyErr)
	// }
	publicKey := `-----BEGIN Public key-----
	
	MQIDAQAB
	-----END Public key-----
	`
	privateKey := `-----BEGIN Private key-----
	
	-----END Private key-----
	`
	// Public key encryption private key decryption
	if err := applyPubEPriD(publicKey, privateKey); err != nil {
		log.Println(err)
	}
	// // Public key decryption private key encryption
	// if err := applyPriEPubD(publicKey, privateKey); err != nil {
	// 	log.Println(err)
	// }
}

// Public key encryption private key decryption
func applyPubEPriD(Pubkey, Pirvatekey string) error {
	pubenctypt, err := PublicEncrypt(`hello world`, Pubkey)
	if err != nil {
		return err
	}
	fmt.Println("Public key encryption:  ", pubenctypt)

	pridecrypt, err := PriKeyDecrypt(pubenctypt, Pirvatekey)
	if err != nil {
		return err
	}
	fmt.Println("private key decryption:  ", pridecrypt)
	if pridecrypt != `hello world` {
		return errors.New(`Decryption failed`)
	}
	return nil
}

// Public key decryption private key encryption
func applyPriEPubD(Pubkey, Pirvatekey string) error {
	prienctypt, err := PriKeyEncrypt(`hello world`, Pirvatekey)
	if err != nil {
		return err
	}
	fmt.Println("private key encryption:  ", prienctypt)

	pubdecrypt, err := PublicDecrypt(prienctypt, Pubkey)
	if err != nil {
		return err
	}
	fmt.Println("Public key decryption:  ", pubdecrypt)
	if pubdecrypt != `hello world` {
		return errors.New(`Decryption failed`)
	}
	return nil
}
